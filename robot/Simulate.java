...
public class Simulate extends JFrame implements MouseListener, MouseMotionListener{
    ...
    public void paint(Graphics g){
        ...
        if (!Double.isNaN(angles[0])){
            last_X = (int)x_click;
            last_Y = (int)y_click;
            y_2 = ArmConstants.LIMB1_LENGTH * Math.cos(Math.toRadians(angles[0]));
            x_2 = ArmConstants.LIMB1_LENGTH * Math.sin(Math.toRadians(angles[0]));
        ...
        g.drawLine(CENTER_X, HEIGHT, CENTER_X - convertInchestoPixel(x_2), HEIGHT + convertInchestoPixel(y_2));
        g.drawLine(CENTER_X - convertInchestoPixel(x_2), HEIGHT + convertInchestoPixel(y_2), last_X, last_Y);
